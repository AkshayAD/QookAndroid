-- Align dim_users email sync and deleted-account lifecycle with the current admin/backend behavior.

-- 1. Keep dim_users aligned with auth for active users.
...
SET
  email = au.email,
  last_active_at = COALESCE(au.last_sign_in_at, d.last_active_at),
  updated_at = NOW()
FROM auth.users au
WHERE d.user_id = au.id
  ...
  AND (
    d.email IS NULL
    OR BTRIM(d.email) = ''
    OR d.email IS DISTINCT FROM au.email
    OR (au.last_sign_in_at IS NOT NULL AND d.last_active_at IS DISTINCT FROM au.last_sign_in_at)
  );

-- 2. Backfill deleted-user markers for already archived accounts.
UPDATE public.dim_users d
SET
  email = COALESCE(NULLIF(BTRIM(d.email), ''), du.email),
  deleted_at = COALESCE(d.deleted_at, du.deleted_at),
  updated_at = NOW()
FROM public.deleted_users du
WHERE d.user_id = du.id
  AND (
...
SET
  deleted_at = COALESCE(us.deleted_at, du.deleted_at),
  status = CASE WHEN us.status = 'pending' THEN us.status ELSE 'cancelled' END,
  cancelled_at = COALESCE(us.cancelled_at, du.deleted_at),
  updated_at = NOW()
FROM public.deleted_users du
WHERE us.user_id = du.id
  AND us.deleted_at IS NULL;

UPDATE public.preference_profiles pp
SET
  deleted_at = COALESCE(pp.deleted_at, du.deleted_at),
  updated_at = NOW()
FROM public.deleted_users du
WHERE pp.user_id = du.id
  AND pp.deleted_at IS NULL;

UPDATE public.user_credits uc
SET
  deleted_at = COALESCE(uc.deleted_at, du.deleted_at),
  updated_at = NOW()
FROM public.deleted_users du
WHERE uc.user_id = du.id
  AND uc.deleted_at IS NULL;

-- 3. New signups must always hydrate dim_users with email and activity fields.
...
DROP TRIGGER IF EXISTS on_auth_user_updated_dim_users ON auth.users;
CREATE TRIGGER on_auth_user_updated_dim_users
AFTER UPDATE OF email, last_sign_in_at ON auth.users
FOR EACH ROW
EXECUTE FUNCTION public.sync_dim_user_from_auth();

-- 4. Deleted accounts should be archived and soft-deleted, not hard-deleted.
